...
import {classWiseactivityFacultyDB} from '../App'
...
class MarksEntry extends Component {
  ...
  handleclass = (value) => {
    ...
    class_value=value;
    if(class_value!==''){
      classWiseactivityFacultyDB.find({
       selector: {class :class_value }
     }).then(function (result) {
        actData=[]
       result.docs.forEach(function(Object){
         actData.push({value:Object.activity,label:Object.activity});
         console.log(actData);
       });
     }).catch(function (err) {
       console.log(err);
  ...
  render () {
    ...
    return (
      <div style={{ flex: 1, overflowX: 'auto', margin: '5% 10% 10% 20%' }}>
        ...
        <Dropdown icon='label'
          source={actData} // to be pulled from class database ( no. of classes of that school)
          onChange={this.handleactivity}
          value={this.state.activity} label='activity' required
        />
